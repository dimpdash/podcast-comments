    ...
    C: Has<XSpanIdString>  + Clone + Send + Sync + 'static,
{
    ...
    async fn add_comment(
        &self,
        param_comment: Option<models::Comment>,
        context: &C) -> Result<AddCommentResponse, ApiError>
    {
        ...
        // Body parameter
        let body = param_comment.map(|ref body| {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = serde_xml_rs::from_str::<models::Comment>(body)
                    .map_err(|e| ApiError(format!("Response body did not match the schema: {}", e)))?;
                Ok(AddCommentResponse::SuccessfulOperation
                    (body)
                )
            }
            405 => {
        ...
        context: &C) -> Result<GetCommentResponse, ApiError>
    {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<Vec<models::Comment>>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(GetCommentResponse::SuccessfulOperation
                    (body)
                )
            }
            code => {
